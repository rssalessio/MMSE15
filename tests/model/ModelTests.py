...
def AccountTest(db):
    ...
    account.setAccountType(AccountType.Employee.value)
    assert(account.getAccountType()==AccountType.Employee.value)
    account.setAccountTeam(AccountTeam.Administration.value)
    assert(account.getAccountTeam()==AccountTeam.Administration.value)
    account.setAccountQualification(AccountQualification.Audio.value)
    assert(account.getAccountQualification()==AccountQualification.Audio.value)
    account.setComment('c')
    assert(account.getComment()=='c')
    account.setDepartment('d')
    assert(account.getDepartment()=='d')
    account.setEmail('name@test.com')
    assert(account.getEmail()=='name@test.com')
    account.setName('name')
    assert(account.getName()=='name')
...
def ClientTest(db):
    ...
    assert(client.getEmail()=='test@test.com')
    client.setAddress('address')
    assert(client.getAddress()=='address')
    client.setBirthDate('10/10/1991')
    assert(client.getBirthDate()=='10/10/1991')
    client.setCity('Stockholm')
    assert(client.getCity()=='Stockholm')
    client.setID(1)
    assert(client.getID()==1)
    client.setPostalCode(16001)
    assert(client.getPostalCode()==16001)
    assert (db.isConnectionOk() == 1)
...
def RequestTest(db):
    ...
    request.setEndDate('10/10/2015')
    assert(request.getEndDate()=='10/10/2015')
    request.setEventType('party')
    assert(request.getEventType()=='party')
    request.setExpectedBudget(1000)
    assert(request.getExpectedBudget()==1000)
    request.setExpectedParticipants(2000)
    assert(request.getExpectedParticipants()==2000)
    request.setPreferences('none')
    assert(request.getPreferences()=='none')
    request.setStartDate('9/10/2015')
    assert(request.getStartDate()=='9/10/2015')
    request.setStatus(RequestStatus.Accepted1.value)
    assert(request.getStatus()==RequestStatus.Accepted1.value)
    assert (db.isConnectionOk() == 1)
...
def ScheduleTest(db):
    ...
    s.setID(1)
    assert(s.getID()==1)
    s.setDate('10/10/2015')
    assert(s.getDate()=='10/10/2015')
    s.setEmployee('test')
    assert(s.getEmployee()=='test')
    s.setEndHour(10)
    assert(s.getEndHour()==10)
    s.setStartHour(9)
    assert(s.getStartHour()==9)
    assert (db.isConnectionOk() == 1)
